...
\end{itemize}
The test case in Section \ref{parallax_tc1} checks that the Parallax source code analysis component generates code as expected. This is followed by a discussion of the results in Section \ref{parallax_test_discussion}.

\subsubsection{Test Case 1} \label{parallax_tc1}
\begin{description}
\item[Description:] The input source code contains a mixture of Parallax library methods and other functions. This test case checks that empty methods are only generated for the Parallax library methods and no include statements are copied across.
\item[Input:] The source code in Appendix \ref{ir_hum_parallax_in}.
\item[Expected Output:] Methods generated with the following signatures:
\begin{itemize}
...
\end{itemize}
No include statments in output.
\item[Actual Output:] See Appendix \ref{parallax_tc1_out}.
\item[Result:] Test case passed.
\end{description}

\subsubsection{Discussion} \label{parallax_test_discussion}
%TODO: discuss

\section{Case Studies} \label{eval_case_studies}
%TODO: add intro
\subsection{Case Study 1 (CS-1)} \label{cs1}
Figure \ref{fig:ir_hum_diagram} shows the Fritzing diagram for the target platform in CS-1. The source code, Fritzing netlist and generated code for this case study can be viewed in Appendices \ref{ir_hum_parallax_in}, \ref{ir_hum_netlist} and \ref{cs1_out} respectively.

% TODO: update the case study now fritz/arduino/parallax analysis moved earlier - can ref unit tests
