...
Evaluation of the software migration system will take the form of case studies at differing levels of complexity.
%TODO: add intro. to how results are displayed in table
...
Figure \ref{fig:cs1_gen_pins} shows the generated variables in the output code. These correspond to the expected variables and have intelligible names except in the case of \codeword{ts2ReadPin1/ts1ReadPin1}. This is because the name of the IR receivers in Fritzing is ``TSOP312.., 314..'' and so this can't be used to generate a useful variable name.  

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.7\linewidth]{graphics/cs1_gen_pins.png}
  \caption{Generated pin variables.}
  \label{fig:cs1_gen_pins}
\end{figure}

\item[Arduino library suggestions:]
The servos and humidity/temp. sensor require the use of libraries in the Arduino code. The appropriate libraries are \codeword{Servo.h} and \codeword{DHT.h} respectively. Figure \ref{fig:selection_options} shows the suggestions shown to the user in the migration process for these parts (with the correct libraries highlighted). Both of the correct libraries were shown in the top 5 suggestions showing that, in this case, LibraryRanker provides a useful heuristic. Furthermore, for the humidity and temperature sensor, the more highly suggested libraries are all for humidity/temp. sensors just not the specific \codeword{DHT22} sensor used in this circuit.
The \codeword{Boards.h} library is ranked higher than \codeword{Servo.h} as it contains macro definitions for variables such as \codeword{MAX_SERVOS} for many different types of board (e.g. Arduino Mega, Primo etc.) and therefore the term ``servo'' appears many times, contributing to a higher \textbf{TF-IDF} rank. A potential improvement to the ranking heuristic would also take file size into account e.g. consider the proportion of the file containing the term ``servo'' as opposed to the raw count.
\begin{figure}[h!]
  \centering
...
\end{figure}
\item[Generated code:]
Two types of compile-time errors were present in the generated output from the migration system. Firstly, a macro definition from the Parallax \codeword{dht22} library (\codeword{CELSIUS}) caused an error as it was not defined in the generated output (see Figure \ref{fig:celsius_error}). Additionally there was no indication that this variable needed to be replaced in the generated code.
\begin{figure}[h!]
  \centering
...
\end{figure}
\item[Manual adaptation/deployment on Arduino:] %TODO
\end{description}

%TODO: analysis of migration - incl reference to tables to say they indicate results
...
\includepdf[pages=1,scale=0.75, pagecommand=\section{CS-1 Parallax Source}\label{cs1_in}]{graphics/ir_humidity_sensors_in.pdf}
\includepdf[pages=2-,scale=0.75, pagecommand={}]{graphics/ir_humidity_sensors_in.pdf}
\includepdf[pages=1,scale=0.75, pagecommand=\section{CS-1 Netlist}\label{cs1_netlist}]{graphics/ir_humidity_sensors_netlist.pdf}
\includepdf[pages=2-,scale=0.75, pagecommand={}]{graphics/ir_humidity_sensors_netlist.pdf}
\includepdf[pages=1,scale=0.75, pagecommand=\section{CS-1 Generated Code}\label{cs1_out}]{graphics/ir_humidity_sensors_out.pdf}
